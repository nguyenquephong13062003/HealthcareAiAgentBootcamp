...
class AIExplainer:
    ...
    Main design:
    1. Build a strong deterministic explanation first.
    2. Optionally use FLAN-T5-Large to rewrite that explanation in more natural
       language.
    3. If model output is weak or fails, safely fall back to the deterministic
       version.
    """
    ...
    def _try_load_model(self):
        try:
            ...
            print(
                "Explainer model could not be loaded. Falling back to "
    ...
    def _build_structured_explanation(self, payload: Dict[str, Any]) -> str:
        ...
        if not top:
            return (
                "The system could not generate a clear health insight from the "
                "current input. "
        ...
        lines.append(
            f"Based on the symptoms entered, the system found a health insight "
            f"that is most closely related to: {diagnosis}."
        )
        lines.append(
            f"The recommended next step is to consult a {specialty} specialist."
        )
        lines.append(
            f"The urgency level for this case is marked as {urgency}."
        )
        lines.append(
            f"Suggested tests include: {tests}."
        )
        lines.append(
            f"General advice from the support system is: {advice}"
        )

        if insurance_text:
            lines.append("")
            lines.append(insurance_text)

        if history_summary:
            lines.append("")
            lines.append(f"History note: {history_summary}")

        if doctors:
            lines.append("")
            lines.append(
                "Suggested doctors based on the selected city and "
                "specialty include:"
            )

            for doc in doctors[:3]:
                lines.append(
                    f"- {doc.get('name', 'Unknown')} in {doc.get('city', 'Unknown')} "
                    f"({doc.get('specialty', 'Unknown')}, rating "
                    f"{doc.get('rating', 'N/A')})"
                )

        lines.append("")
        lines.append(
            "This explanation is for educational decision-support purposes only "
    ...
    def _build_rewrite_prompt(self, structured_text: str) -> str:
        prompt = f"""
Rewrite the following healthcare support explanation in simple, calm, patient-friendly English.
...
"""
    ...
    def explain_model_only(self, payload: Dict[str, Any]) -> str:
        """
        Debug helper: returns only the raw FLAN rewrite result.
        """
        if not self.model_loaded:
    ...
    def explain(self, payload: Dict[str, Any], use_ai: bool = False) -> str:
        """
        Default:
        - use_ai=False -> deterministic explanation
        - use_ai=True -> try FLAN rewrite, else fallback safely
        """
        structured_text = self._build_structured_explanation(payload)

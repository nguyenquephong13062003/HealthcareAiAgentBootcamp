...
def format_doctors(doctors):
    ...
    for doc in doctors:
        lines.append(
            f"{doc['name']} | {doc['specialty']} | {doc['city']} | Rating: "
            f"{doc['rating']}"
        )
...
def format_history(history):
    ...
    for idx, visit in enumerate(visits, start=1):
        ...
        lines.append(
            f"Insurance estimate: {visit.get('insurance_price', 'Unknown')}"
        )
...
def format_detailed_analysis(result):
    ...
    lines.append("-" * 40)
    lines.append(
        f"Matched symptom pattern: {top.get('symptom_pattern', 'Unknown')}"
    )
    lines.append(f"Health insight: {top.get('diagnosis', 'Unknown')}")
    lines.append(
        f"Recommended specialty: {top.get('specialty', 'Unknown')}"
    ...
    lines.append(
        f"Suggested tests: {top.get('recommended_tests', 'Unknown')}"
    )
    lines.append(f"Advice: {top.get('advice', 'Unknown')}")
    lines.append(
        f"Similarity confidence: {top.get('confidence', 0):.4f}"
    )
